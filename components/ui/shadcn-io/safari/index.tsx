...
export interface SafariProps extends SVGProps<SVGSVGElement> {
  ...
  height?: number;
  mode?: SafariMode;
  autoFetch?: boolean; // New prop to enable automatic fetching
}
...
export function Safari({
  ...
  useEffect(() => {
    if (autoFetch && url && !imageSrc && !videoSrc) {
      fetchScreenshot(url);
    }
  }, [url, autoFetch, imageSrc, videoSrc]);

  const fetchScreenshot = async (targetUrl: string) => {
    setLoading(true);
    try {
      // Option 1: Use a screenshot API service
      const screenshotUrl = `https://api.screenshotone.com/take?url=${encodeURIComponent(
        targetUrl
      )}&viewport_width=1200&viewport_height=700&format=jpg&access_key=YOUR_API_KEY`;
  ...
  const displayImage = imageSrc || screenshot;
  ...
  return (
    ...
    >
      <g clipPath="url(#path0)">
        ...
        {/* URL Text */}
        <g className="mix-blend-luminosity">
          <text x={300} y={30} fontSize="12" fill="#A3A3A3">
            {url || "Enter URL"}
          </text>
        </g>

        {/* Toolbar Icons (only in default mode) */}
        {mode === "default" ? (
          <>
            {/* Shield Icon */}
            <g className="mix-blend-luminosity">
